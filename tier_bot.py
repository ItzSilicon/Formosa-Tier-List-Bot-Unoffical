...
import sqlite3
import os
from discord import app_commands,Interaction,Message,Member,Embed,Colour,DMChannel,Thread
...
import logging
from entities import db_backup,get_modes_dict,get_examiner_dict,new_conn,get_tier_table,Player,EntityException,get_players_amount,get_banned_amount,get_tier_list_amount,fetch_core_rank,fetch_overall_rank,query_to_dataframe
...
import re
from config import *
from chatbot import chat_via_interaction,chat_via_mention,set_gemini_api_key,remove_gemini_api_key,chatuser_info,ChatUser,async_session,engine
import asyncio
...
TLC_BOT_ID=1175318571216285758
### ON MESSAGE ###
@bot.event
async def on_message(message:Message):
    if message.author.id==bot.user.id:
        return
    if message.author.bot and not message.author.id==TLC_BOT_ID: #Tier List Comprehensive Bot
    ...
    if message.guild:
        ...
        if message.guild.id==DEV_GUILD.id:
            ...
            if message.channel.category.id in ticket_cate and isinstance(message.channel,Thread) and message.author.id in (TLC_BOT_ID,bot.owner_id): #Sword,Axe,NPot,SMP,Mace
                logging.info("Entering Tick analyzing mode")
                if message.channel.name=="問題答案":
                    logging.info("Message from answer channel")
                    logging.info(f"Message content:\n {message.content}")
                    if "## 區域" in message.content.split("\n")[0]:
                        area=message.content.split("\n")[1].replace("* ","").strip()
                        try:
                            async for msg in message.channel.history(limit=1,oldest_first=True):  
                                player_name_message=msg
                            if  player_name_message.content.split("\n")[0]=="## 遊戲ID": #and player_name_message.author.id==TLC_BOT_ID   
                                name=player_name_message.content.split("\n")[1].replace("* ","").strip()
                        except Exception as e:
                            logging.exception(e)
                            return
                        mode_id=ticket_cate[message.channel.category.id]
                        mode=data_query("SELECT zh_tw FROM mode WHERE mode_id = ?",(mode_id,))
                        outer_channel=message.channel.parent
                        ticket_requester=None
                        outer_channel_name=outer_channel.name
                        target_name=outer_channel_name.split("・")[1].strip()
                        logging.info(f"Outer channel name: {outer_channel_name}, Target name: {target_name}")
                        try:
                            async for msg in outer_channel.history(limit=1,oldest_first=True):  
                                bot_first_message=msg
                            logging.info(f"Bot first message: {bot_first_message}")
                            if bot_first_message.author.id==TLC_BOT_ID:
                                embed_bot_first_message=bot_first_message.embeds[0]
                                if embed_bot_first_message.description:
                                    logging.info(f"Embed description of bot first message: {embed_bot_first_message.description}")
                                    if "> 請先填寫完成問題\n> 當考官空閒時會主動@請勿直接私訊考官或其他工作人員" in embed_bot_first_message.description:
                                        logging.info("Target message is from this message")
                                        first_line=embed_bot_first_message.description.split("\n")[0]
                                        logging.info(f"First line of description: {first_line}")
                                        ticket_requester_id=int(first_line[4:-1])
                                        logging.info(f"Ticket requester id: {ticket_requester_id}")
                                        ticket_requester=await bot.fetch_user(ticket_requester_id)
                                        if ticket_requester.name==target_name:
                                            logging.info(f"{ticket_requester.name=} is the same as {target_name=}")
                                        else:
                                            logging.warning(f"{ticket_requester.name=} is not the same as {target_name=}")

                        except Exception as e:
                            logging.exception(e)
                        
                        logging.info(f"Outer channel name: {outer_channel_name}")
                        logging.info(f"Ticker requester: {ticket_requester}")
                        dm=await owner_user()
                        embed=Embed(title="Quick Overview",description=f"快速了解考試者 **{name}** 的資訊",color=Colour.green())
                        embed.set_footer(text="本功能在測試開發中，供預覽")
                        try:
                        ...
                        logging.info(f"{player.is_banned=}")
                        if player.is_banned:
                            error.append("❌ 封禁中")
                        else:
                            info.append("✅ 未封禁")
                        if ticket_requester:
                            time_utc=datetime.datetime.now(tz=datetime.timezone.utc)
                            create_date=ticket_requester.created_at
                            embed.add_field(name="Discord 帳號建立累積時間",value=f"{(time_utc-create_date).days} 天",inline=False)
                            logging.info(f"Ticket requester create date: {create_date.strftime('%Y-%m-%d %H:%M:%S')}")
                            if player.discord_user_id:
                                if player.discord_user_id != ticket_requester.id:
                                    warning.append("⚠️ 開單的Discord帳號不是該Minecraft帳號連結的Discord帳號")
                                    if create_date>(time_utc-datetime.timedelta(days=365)):
                                        warning.append("⚠️ 帳號未開設滿一年，高機率為小帳")
                                else:
                                    if create_date>(time_utc-datetime.timedelta(days=365)):
                        ...
                        if error:
                            embed.add_field(name="❌ 不符合考試要求",value="\n".join(error)+"\n"+"\n".join(warning)+"\n"+"\n".join(info),inline=False)
                            embed.colour=Colour.red()
                        elif warning:
                            embed.add_field(name="⚠️ 注意",value="\n".join(warning)+"\n"+"\n".join(info),inline=False)
                            embed.colour=Colour.yellow()
                        else:
                            embed.add_field(name="✅ 符合基本申請考試要求",value="\n".join(info),inline=False)
                        
                        embed.set_thumbnail(url=player.head_pic_url)
                        await dm.send(embed=embed)
                        await outer_channel.send(embed=embed)
                    else:
                        return
            ...
            if bot.user.mention in message.content:
                ...
                if code:
                    if code == 2:
                        ...
                        embed = Embed(
                            title="發生錯誤",
                            description=get_error_code(code),
                            color=Colour.red()
                        )
                        embed.set_footer(
                            text="本訊息將會於10秒後自動刪除",
...
async def send_message(interaction: Interaction,channel_id:str="0",msg:str="測試"):
    command_execute_log_warpper(interaction.command,locals())
...
async def link_hypixel(interaction: Interaction, api_key:str,player_or_uuid:str):
    command_execute_log_warpper(interaction.command,{})
...
async def tier(interaction: Interaction,player_or_uuid:str=""):
    ...
    try:
        ...
        if interaction.guild_id!=990378958501584916 and rd()>0.7:
            ...
            await interaction.followup.send(embed=embed,content="[᠌ㅤ](https://discord.gg/hamescZvtP)")
        else:
...
async def update_tier(interaction: Interaction,player:str,mode:Choice[int],tier:Choice[int],is_retired:bool=False,inform_discord_user:Member=None,reason:str=None):
    ...
    orginal_tier=player_to_update.get_tier(str(mode.value))
...
async def add_test_record(interaction:Interaction, examinee:str, mode:Choice[int],examiner:str,examinee_score:int,examiner_score:int,new_tier:Choice[int],date:str="today",examinee_discord:Member=None,input_test_id:str="Default",orginal_tier:Choice[int]=None,do_update_tier:bool=True,ps:str=""):
    ...
    # logging.info(f"{interaction.user.id=},{interaction.user.name=}")
    command_execute_log_warpper(interaction.command,locals())
    ...
    # logging.info(f"{in_test_report_chennel=},{interaction.channel.name=}")
    examinee=Player(examinee)
    examiner=Player(examiner)
    fetch_using_minecraft_linked=False
        ...
        if role == "admin":
            ...
            if in_test_report_chennel:
                ...
                if linked:
                    ...
                    await interaction.followup.send(embed=Embed(title="你尚未連結Discord帳號",description="請先連結Discord帳號，可以向開發者申請手動連結"),ephemeral=True)
                    return
            else:
            ...
            await interaction.followup.send(embed=Embed(title="你沒有權限",description="只有開發者、管理員和考官可以使用該指令。如果你是考官，請聯繫開發者開通權限"),ephemeral=True)
            return
                
    if not examinee_discord:
        if examinee.discord_user_id:
            examinee_discord = await bot.fetch_user(examinee.discord_user_id)
            fetch_using_minecraft_linked=True
        else:
            logging.info("examiner did not provide dcmember and examinee is not linked")
            await interaction.followup.send(embed=Embed(title="受試者未連結Discord帳號",description="請提供受試者Discord用戶"),ephemeral=True)
            return
        ...
        if examiner.discord_user_id:
            ...
            if examiner_dm_channel:
                try:
                    ...
                    public_channel=await bot.fetch_channel(990396736335921263)
                    await public_channel.send(content=f"<@{examiner.discord_user_id}>，我無法私訊予您，所以在此通知",embeds=[test_info,Embed(title="!!請仔細核對以上成績資訊!!",colour=Colour.red(),description=f"**考官您好：**\n成績已登記入資料庫！如果以上資訊有問題，或者是後續考試成績需更改之情形，請私訊開發人員/資料庫管理人員 <@{bot.owner_id}>，感謝您的合作！")])
            else:
                await dm.send(content=f"考官 {examiner.name} 沒有連結Discord帳號")
        return
...
@app_commands.checks.cooldown(1, 5, key=lambda i: i.user.id)
@bot.tree.command(name="remove_link", description="手動移除連結Discord - Minecraft")
async def remove_link(interaction: Interaction,player_or_uuid:str=None,discord_user:Member=None):
    command_execute_log_warpper(interaction.command,locals())
    await interaction.response.defer(ephemeral=True)
    if interaction.user.id != bot.owner_id:
        raise CommandException("權限不足","請勿使用該指令")
    ...
    dev_dm=await owner_user()
    if not player_or_uuid and not discord_user:
        raise CommandException("參數錯誤","請輸入玩家名稱或Discord ID")
        return
    elif player_or_uuid:
        player=Player(player_or_uuid)
        if player.discord_user_id:
            discord_user=await bot.fetch_user(player.discord_user_id)
            data_query(f"DELETE FROM discord_minecraft WHERE discord_user_id = ? OR minecraft_uuid = ?",(player.discord_user_id,player.uuid),do_commit=True)
        else:
            await interaction.followup.send(f"未找到玩家 {player.name} ({player.uuid}) 的Discord帳戶連結",ephemeral=True)
            return
    elif discord_user:
        uuid = data_query("SELECT minecraft_uuid FROM discord_minecraft WHERE discord_user_id = ?",(discord_user.id))
        if uuid:
            player=Player(uuid)
            data_query(f"DELETE FROM discord_minecraft WHERE discord_user_id = ? OR minecraft_uuid = ?",(discord_user.id,uuid),do_commit=True)
        else:
            await interaction.followup.send(f"未找到Discord帳戶 {discord_user.mention} ({discord_user.id}) 的Minecraft帳戶連結",ephemeral=True)
            return
    else:
        raise CommandException("太多參數","請擇一輸入玩家名稱或Discord ID")
        return
    
    await dev_dm.send(f"已移除 與Minecraft 玩家 {player.name} ({player.uuid}) 、 {discord_user.mention} ({discord_user.id}) 任何之連結。")
    await interaction.followup.send(content="操作成功。",ephemeral=True)
    return

@app_commands.checks.cooldown(1, 5, key=lambda i: i.user.id)
@bot.tree.command(name="remove_test_record", description="移除考試紀錄")
async def remove_test_record(interaction: Interaction,test_id:str,recover_tier:bool=True):
    command_execute_log_warpper(interaction.command,locals())
    await interaction.response.defer()
    if interaction.user.id != bot.owner_id:
        raise CommandException("權限不足","請勿使用該指令")
    ...
    embed=Embed(colour=Colour.orange(),title="移除考試紀錄")
    data = query_to_dataframe("SELECT * FROM test_records WHERE test_id = ?",(test_id,))
    data:dict
    if data:
        logging.info(data)
        examinee=Player(data["examinee"])
        if recover_tier:
            valid = data["outcome_tier"] == examinee.get_tier(data["mode"])
            logging.info(f"{data["outcome_tier"]=},{examinee.get_tier(data['mode'])=}")
            if valid:
                examinee.update_tier(data["mode"],data["orginal_tier"])
                embed.add_field(name="還原玩家階級",value=f"玩家 {examinee.name} (`{examinee.uuid}`)\n{data['mode']} 模式 Tier 已從 {data['outcome_tier']} 還原至 {data['orginal_tier']}",inline=False)
            else:
                raise CommandException("❓無法還原","考試結果與現在玩家階級不符")
                return
        embed.add_field(name="刪除考試紀錄",value=f"已刪除考試紀錄 `{test_id}`",inline=False)
        data_query("DELETE FROM tests WHERE test_id = ?",(test_id,),do_commit=True)
        
        await interaction.followup.send(embed=embed)
        return
    else:
        raise CommandException("無效 Test ID","找不到該考試ID")
...
async def ai_chat(interaction: Interaction,text:str):
    command_execute_log_warpper(interaction.command,locals())
    if len(text)>500:
        raise CommandException("AI提示詞太長","請控制在500字以內")
    command_execute_log_warpper(interaction.command,locals())
    await interaction.response.defer()
...
### AUTO COMPLETE ###
@remove_link.autocomplete("player_or_uuid")
...
@test_history.autocomplete("test_id_like")
@remove_test_record.autocomplete("test_id")
async def auto_complete_test_id(interaction: Interaction, current: str):
    logging.info(f"triggeer AC-test_id")
    l=query_to_dataframe("SELECT * FROM test_records ORDER BY test_date DESC",do_format=False)
    logging.info(f"{l=}")
    logging.info(f"{[x.test_id for x in l.itertuples()][:5]=}")    
    if current:
        match_=query_to_dataframe(f"SELECT *, test_id||test_date||mode||examinee||examiner as filter FROM test_records WHERE filter LIKE ? ORDER BY test_date DESC",(f"%{current}%",),do_format=False)
        return [app_commands.Choice(name=f"{x.test_id} | {x.test_date} / {x.mode} {x.examinee} {x.examinee_grade} : {x.examiner_grade} {x.examiner} / {x.orginal_tier} → {x.outcome_tier}",value=x.test_id) for x in match_.itertuples()][:25]
    else:
        return [app_commands.Choice(name=f"{x.test_id} | {x.test_date} / {x.mode} {x.examinee} {x.examinee_grade} : {x.examiner_grade} {x.examiner} / {x.orginal_tier} → {x.outcome_tier}",value=x.test_id) for x in l.itertuples()][:25]
...
async def on_tree_error(interaction: Interaction, error: app_commands.AppCommandError):
    ...
    enable_ephemeral= not await check_ephemeral(interaction)
    ...
    if isinstance(error, app_commands.CommandOnCooldown):
        ...
        await send(embed=embed, ephemeral=enable_ephemeral)
    elif isinstance(error, app_commands.MissingPermissions):
        await send("你沒有使用這個指令的權限", ephemeral=enable_ephemeral)
        return
    elif isinstance(error, EntityException):
        error:EntityException
        embed=Embed(colour=Colour.red(),title="內部操作錯誤")
        if error.solution:
            embed.add_field(name=error.message,value=error.solution)
        else:
